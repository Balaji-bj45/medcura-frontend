...
// âœ… Lucide-safe icon registry (works on ALL versions)
const iconMap = {
  Activity,
  Accessibility,
  BedDouble,
  Dumbbell,
  Wind, // used for lungs / oxygen / respiratory
}

// âœ… Normalize icon names from API / constants
function getIconByName(iconName) {
  if (!iconName) return Activity

  // handle backend mistakes safely
  if (iconName === 'Lungs' || iconName === 'Lung') return Wind
    ...
    <section className="bg-white px-4 py-16 sm:px-6 lg:px-10 lg:py-20">
      <div className="mx-auto w-full max-w-7xl">
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <h2 className="heading-font text-3xl font-semibold text-[#0e336b] sm:text-4xl">
              Shop by Category
            </h2>
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {visibleCategories.map((category, index) => {
            ...
            const Icon = getIconByName(category.icon || fallback?.icon)
            ...
            return (
              ...
              >
                ...
                >
                  <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[#d8f4fa] text-[#0e336b]">
                    <Icon className="h-6 w-6" />
                  </div>

                  <h3 className="mt-4 text-lg font-semibold text-[#0e336b]">
                    {category.name}
                  </h3>

                  <p className="mt-2 text-sm leading-relaxed text-[#0e336b]/70">
                    {description}
                  </p>
        ...
        {loading && (
          <p className="mt-4 text-sm text-[#0e336b]/60">
            Loading categories...
          </p>
        )}
      </div>
